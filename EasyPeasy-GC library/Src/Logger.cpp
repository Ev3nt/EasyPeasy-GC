...
namespace Logger {
	...
	void Log(std::string info, LEVEL level, LPCSTR file, int line) {
		...
		//if (!color.empty()) {
			SYSTEMTIME time;
			GetLocalTime(&time);
			info = Utils::format("[%02d:%02d:%02d]%s [%s:%d]: %s", time.wHour, time.wMinute, time.wSecond, !message.empty() ? Utils::format(" [%s]", message.data()).data() : "", filename.data(), line, info.data());
		//}
